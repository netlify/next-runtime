  ...
  function runTests() {
    ...
    it('should use cache busting when loading css (dev only)', async () => {
      ...
      links.each((_, link) => {
        ...
        isDev ? expect(href).toMatch(/\?ts=/) : expect(href).not.toMatch(/\?ts=/)
      })
    })
    ...
    describe('css support', () => {
      ...
      describe('special entries', () => {
        it('should include css imported in loading.js', async () => {
          ...
          expect(html).toMatch(/<link rel="stylesheet" href="(.+)\.css(\?ts=\d+)?"\/><h2>Loading...<\/h2>/)
        })
    ...
    describe('client pages', () => {
      it('should support global sass/scss inside client pages', async () => {
        ...
        await waitFor(5000)
        // .sass
        expect(await browser.eval(`window.getComputedStyle(document.querySelector('#sass-client-page')).color`)).toBe(
          'rgb(245, 222, 179)',
        )
        // .scss
        expect(await browser.eval(`window.getComputedStyle(document.querySelector('#scss-client-page')).color`)).toBe(
          'rgb(255, 99, 71)',
        )
      })
