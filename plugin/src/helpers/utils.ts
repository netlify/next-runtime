...
export const isNextAuthInstalled = (): boolean => {
  try {
    // eslint-disable-next-line import/no-unassigned-import, import/no-unresolved, node/no-missing-require, node/no-extraneous-require, import/no-extraneous-dependencies
    require('next-auth')
    return true
  } catch {
