...
export class Bridge {
  ...
  async launcher(event, context) {
    ...
    if ('payloads' in normalizedEvent && Array.isArray(normalizedEvent.payloads)) {
      let statusCode = 200
      /**
      ...
      for (let i = 0; i < normalizedEvent.payloads.length; i++) {
        ...
        combinedBody += `content-type: ${response.headers['content-type'] || 'text/plain'}${CLRF}${CLRF}`
        ...
        if (response.statusCode !== 200) {
          headers[`x-vercel-payload-${i + 1}-status`] = response.statusCode + ''
        }
      ...
      allHeaderKeys.forEach((curKey) => {
        ...
        const canDedupe = separateHeaders.every((headers) => headers[curKey] === curValue)
        ...
        if (canDedupe) {
          ...
          separateHeaders.forEach((curHeaders, idx) => {
            if (curHeaders[curKey]) {
              headers[`x-vercel-payload-${idx + 1}-${curKey}`] = curHeaders[curKey]
            }
          })
        }
      })
  ...
  async handleEvent(normalizedEvent) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (responseCallbackCipher && responseCallbackCipherKey && responseCallbackCipherIV) {
...
function* getHeadersIterator(headers) {
  for (const [name, value] of Object.entries(headers)) {
    if (value === undefined) {
      console.error(`Skipping HTTP request header "${name}" because value is undefined`)
      continue
    }
