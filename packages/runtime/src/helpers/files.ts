...
const nextServerReplacements: Array<[string, string]> = [
  [
    ...
    `getMiddlewareManifest() {\n        if (this.minimalMode || !(process.env.NEXT_DISABLE_NETLIFY_EDGE !== 'false' && process.env.NEXT_DISABLE_NETLIFY_EDGE !== '0')) return null;`,
  ],
  [
    `generateCatchAllMiddlewareRoute(devReady) {\n        if (this.minimalMode) return []`,
    `generateCatchAllMiddlewareRoute(devReady) {\n        if (this.minimalMode || !(process.env.NEXT_DISABLE_NETLIFY_EDGE !== 'false' && process.env.NEXT_DISABLE_NETLIFY_EDGE !== '0')) return [];`,
  ],
  [
    `generateCatchAllMiddlewareRoute() {\n        if (this.minimalMode) return undefined;`,
    `generateCatchAllMiddlewareRoute() {\n        if (this.minimalMode || !(process.env.NEXT_DISABLE_NETLIFY_EDGE !== 'false' && process.env.NEXT_DISABLE_NETLIFY_EDGE !== '0')) return undefined;`,
  ],
  [
    `getMiddlewareManifest() {\n        if (this.minimalMode) {`,
    `getMiddlewareManifest() {\n        if (!this.minimalMode && (process.env.NEXT_DISABLE_NETLIFY_EDGE !== 'false' && process.env.NEXT_DISABLE_NETLIFY_EDGE !== '0')) {`,
  ],
]
