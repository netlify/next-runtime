...
const runTests = (isDev = false) => {
  ...
  it('should have correct encoding for params with catchall rewrite', async () => {
    const html = await renderViaHTTP(appPort, '/catchall-rewrite/hello%20world%3Fw%3D24%26focalpoint%3Dcenter?a=b')
    const $ = cheerio.load(html)
    expect(JSON.parse($('#__NEXT_DATA__').html()).query).toEqual({
      a: 'b',
  ...
  it('should handle encoded value in the pathname correctly', async () => {
    const res = await fetchViaHTTP(appPort, '/redirect/me/to-about/' + encodeURI('\\google.com'), undefined, {
      redirect: 'manual',
    })
  ...
  if (!isDev) {
    it('should output routes-manifest successfully', async () => {
      ...
      expect(manifest).toEqual({
        ...
        dataRoutes: [
          {
            dataRouteRegex: normalizeRegEx(`^/_next/data/${escapeRegex(buildId)}/blog\\-catchall/(.+?)\\.json$`),
            namedDataRouteRegex: `^/_next/data/${escapeRegex(buildId)}/blog\\-catchall/(?<slug>.+?)\\.json$`,
        ...
        redirects: [
          ...
          },
          {
            ...
            regex: normalizeRegEx('^(?!\\/_next)\\/redirect\\/me\\/to-about(?:\\/([^\\/]+?))(?:\\/)?$'),
          ...
          },
          {
            ...
            statusCode: 301,
            regex: normalizeRegEx('^(?!\\/_next)\\/docs\\/router-status(?:\\/([^\\/]+?))(?:\\/)?$'),
          },
          {
            source: '/docs/github',
          ...
          },
          {
            ...
            statusCode: 301,
            regex: normalizeRegEx('^(?!\\/_next)\\/docs\\/v2\\/advanced(?:\\/(.*))(?:\\/)?$'),
          },
          {
            source: '/hello/:id/another',
            destination: '/blog/:id',
            statusCode: 307,
            regex: normalizeRegEx('^(?!\\/_next)\\/hello(?:\\/([^\\/]+?))\\/another(?:\\/)?$'),
          },
          {
            source: '/redirect1',
            destination: '/',
          ...
          },
          {
            ...
            regex: normalizeRegEx('^(?!\\/_next)\\/query-redirect(?:\\/([^\\/]+?))(?:\\/([^\\/]+?))(?:\\/)?$'),
            source: '/query-redirect/:section/:name',
            statusCode: 307,
          },
          {
            destination: '/got-unnamed',
            regex: normalizeRegEx('^(?!\\/_next)\\/unnamed(?:\\/(first|second))(?:\\/(.*))(?:\\/)?$'),
            source: '/unnamed/(first|second)/(.*)',
            statusCode: 307,
          },
          {
            destination: '/:0',
            regex: normalizeRegEx('^(?!\\/_next)\\/named-like-unnamed(?:\\/([^\\/]+?))(?:\\/)?$'),
          ...
          },
          {
            ...
            regex: normalizeRegEx('^(?!\\/_next)\\/docs(?:\\/(integrations|now-cli))\\/v2(.*)(?:\\/)?$'),
          ...
          },
          {
            ...
            regex: normalizeRegEx('^(?!\\/_next)(?:\\/([^\\/]+?))\\/has-redirect-5(?:\\/)?$'),
        ...
        headers: [
          ...
          },
          {
            ...
            regex: normalizeRegEx('^\\/with-params\\/url(?:\\/((?:[^\\/]+?)(?:\\/(?:[^\\/]+?))*))?(?:\\/)?$'),
          ...
          },
          {
            ...
            regex: normalizeRegEx('^\\/with-params\\/url2(?:\\/((?:[^\\/]+?)(?:\\/(?:[^\\/]+?))*))?(?:\\/)?$'),
        ...
        rewrites: {
          ...
          afterFiles: [
            ...
            },
            {
              ...
              regex: normalizeRegEx('^\\/test-overwrite(?:\\/([^\\/]+?))(?:\\/([^\\/]+?))(?:\\/)?$'),
            },
            {
            ...
            {
              destination: '/with-params?first=:section&second=:name',
              regex: normalizeRegEx('^\\/query-rewrite(?:\\/([^\\/]+?))(?:\\/([^\\/]+?))(?:\\/)?$'),
              source: '/query-rewrite/:section/:name',
            },
            {
              destination: '/_next/:path*',
              regex: normalizeRegEx('^\\/hidden\\/_next(?:\\/((?:[^\\/]+?)(?:\\/(?:[^\\/]+?))*))?(?:\\/)?$'),
            ...
            {
              destination: '/with-params',
              regex: normalizeRegEx('^\\/unnamed-params\\/nested(?:\\/(.*))(?:\\/([^\\/]+?))(?:\\/(.*))(?:\\/)?$'),
              source: '/unnamed-params/nested/(.*)/:test/(.*)',
            },
            {
              destination: '/with-params',
              regex: normalizeRegEx('^\\/catchall-rewrite(?:\\/((?:[^\\/]+?)(?:\\/(?:[^\\/]+?))*))?(?:\\/)?$'),
              source: '/catchall-rewrite/:path*',
            },
            {
              destination: '/with-params?another=:path*',
              regex: normalizeRegEx('^\\/catchall-query(?:\\/((?:[^\\/]+?)(?:\\/(?:[^\\/]+?))*))?(?:\\/)?$'),
...
describe('Custom routes', () => {
  ...
  beforeAll(async () => {
    ...
    await fs.writeFile(nextConfigPath, nextConfigRestoreContent.replace(/__EXTERNAL_PORT__/, externalServerPort))
  })
  afterAll(async () => {
  ...
  describe('dev mode', () => {
    ...
    beforeAll(async () => {
      ...
      await fs.writeFile(nextConfigPath, nextConfigContent.replace('// no-rewrites comment', 'return []'))
  ...
  describe('no-op rewrite', () => {
    ...
    it('should not error for no-op rewrite and auto export dynamic route', async () => {
      ...
      await check(() => browser.eval(() => document.documentElement.innerHTML), /auto-export.*?my-slug/)
    })
  })
  ...
  describe('export', () => {
    ...
    beforeAll(async () => {
      ...
      const exportResult = await nextExport(appDir, { outdir: join(appDir, 'out') }, { stderr: true })
  ...
  describe('serverless mode', () => {
    beforeAll(async () => {
      nextConfigContent = await fs.readFile(nextConfigPath, 'utf8')
      await fs.writeFile(nextConfigPath, nextConfigContent.replace(/\/\/ target/, 'target'), 'utf8')
  ...
  describe('raw serverless mode', () => {
    ...
    it('should apply rewrites in lambda correctly for API route', async () => {
      const data = JSON.parse(await renderViaHTTP(appPort, '/api-hello-param/first'))
      expect(data).toEqual({
        query: {
          name: 'first',
          hello: 'first',
    ...
    it('should apply rewrites in lambda correctly for dynamic API route', async () => {
      const data = JSON.parse(await renderViaHTTP(appPort, '/api-dynamic-param/first'))
      expect(data).toEqual({
        query: {
          slug: 'first',
          hello: 'first',
  ...
  describe('should load custom routes when only one type is used', () => {
    const runSoloTests = (isDev) => {
      ...
      it('should work with just headers', async () => {
        nextConfigContent = await fs.readFile(nextConfigPath, 'utf8')
        await fs.writeFile(nextConfigPath, nextConfigContent.replace(/(async (?:redirects|rewrites))/g, '$1s'))
        await buildAndStart()
      ...
      it('should work with just rewrites', async () => {
        nextConfigContent = await fs.readFile(nextConfigPath, 'utf8')
        await fs.writeFile(nextConfigPath, nextConfigContent.replace(/(async (?:redirects|headers))/g, '$1s'))
        await buildAndStart()
      ...
      it('should work with just redirects', async () => {
        nextConfigContent = await fs.readFile(nextConfigPath, 'utf8')
        await fs.writeFile(nextConfigPath, nextConfigContent.replace(/(async (?:rewrites|headers))/g, '$1s'))
        await buildAndStart()
